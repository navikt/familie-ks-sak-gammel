...
COMMENT ON COLUMN SO_KRAV.BARN_HJEMME IS 'Barnet er ikke adoptert, i fosterhjem eller på institusjon.';
COMMENT ON COLUMN SO_KRAV.BOR_SAMMEN_MED_BARNET IS 'Søker bor i Norge sammen med barnet';
COMMENT ON COLUMN SO_KRAV.IKKE_AVTALT_DELT_BOSTED IS 'Søker og annen part har ikke avtalt delt bosted.';
COMMENT ON COLUMN SO_KRAV.BARN_I_NORGE IS 'Søker og barnet skal ikke oppholde seg i utlandet mer enn tre måneder de neste tolv månedene.';

CREATE TABLE SO_SOKNAD
(
    ID                 serial primary key,
    INNSENDT_TIDSPUNKT timestamp(3)                     not null,
    OPPGITT_KRAV_ID    bigint REFERENCES SO_KRAV (id)   NOT NULL,
